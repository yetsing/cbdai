...
const char* dai_value_ts(DaiValue value) {
    switch (value.type) {
    case DaiValueType_undefined:
        return "undefined";
    ...
    case DaiValueType_obj:
        return dai_object_ts(value);
    default:
        return "unknown";
    }
}

void dai_print_value(DaiValue value) {
    switch (value.type) {
    case DaiValueType_undefined:
        dai_log("undefined");
        break;
    case DaiValueType_nil:
        ...
        break;
    case DaiValueType_int:
        dai_log("%" PRId64, value.as.intval);
        break;
    case DaiValueType_bool:
        if (AS_BOOL(value)) {
            dai_log("true");
...
bool dai_value_equal(DaiValue a, DaiValue b) {
    if (a.type != b.type) {
        return false;
    }
    switch (a.type) {
...
void DaiValueArray_init(DaiValueArray* array) {
    array->capcity = 0;
    array->count = 0;
    array->values = NULL;
}

void DaiValueArray_write(DaiValueArray* array, DaiValue value) {
...
void DaiValueArray_reset(DaiValueArray* array) {
    FREE_ARRAY(DaiValue, array->values, array->capcity);
    DaiValueArray_init(array);
}

void DaiValueArray_copy(DaiValueArray* src, DaiValueArray* dst) {
