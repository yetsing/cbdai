...
dai_utf8_decode(const char *str, dai_rune_t *rune) {
    ...
    *rune = DAI_INVALID_RUNE;
    ...
    case 4: {
        *rune = (dai_rune_t) ((str[0] & 0x7) << 18) | ((str[1] & 0x3F) << 12) | ((str[2] & 0x3F) << 6) | (str[3] & 0x3F);
        if (*rune < 0x10000) {
            return -1;
        }
        return 4;
    ...
    case 5: {
        *rune = (dai_rune_t) ((str[0] & 0x3) << 24) | ((str[1] & 0x3F) << 18) | ((str[2] & 0x3F) << 12) | ((str[3] & 0x3F) << 6) | (str[4] & 0x3F);
        if (*rune < 0x200000) {
            return -1;
        }
        return 5;
    ...
    case 6: {
        *rune = (dai_rune_t) ((str[0] & 0x1) << 30) | ((str[1] & 0x3F) << 24) | ((str[2] & 0x3F) << 18) | ((str[3] & 0x3F) << 12) | ((str[4] & 0x3F) << 6) | (str[5] & 0x3F);
        if (*rune < 0x4000000) {
            return -1;
        }
        return 6;
