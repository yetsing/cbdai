...
static DaiValue dai_default_get_property(DaiVM* vm, DaiValue receiver, DaiObjString* name) {
    dai_error("PropertyError: '%s' object has no property '%s'", dai_value_ts(receiver), name->chars);
    assert(false);
}

static DaiValue dai_default_set_property(DaiVM* vm, DaiValue receiver, DaiObjString* name,
                                         DaiValue value) {
    dai_error("PropertyError: '%s' object has no property '%s'", dai_value_ts(receiver), name->chars);
    assert(false);
}

static DaiObj* allocate_object(DaiVM* vm, size_t size, DaiObjType type) {
...
const char* DaiObjFunction_name(DaiObjFunction* function) { return function->name->chars; }
...
const char* DaiObjClosure_name(DaiObjClosure* closure) { return closure->function->name->chars; }

// #region 类与实例
static DaiValue DaiObjInstance_init(DaiValue receiver, int argc, DaiValue* argv) {
...
static bool DaiObjInstance_get_method(DaiVM* vm, DaiObjClass *klass, DaiObjString* name, DaiValue *method) {
    while (klass != NULL) {
...
static DaiValue DaiObjInstance_get_property(DaiVM* vm, DaiValue receiver, DaiObjString* name) {
    ...
    // todo Exception 将错误也作为一种值返回
    dai_error("PropertyError: '%s' object has no property '%s'", dai_object_ts(receiver), name->chars);
    assert(false);
    return UNDEFINED_VAL;
};
static DaiValue DaiObjInstance_set_property(DaiVM* vm, DaiValue receiver, DaiObjString* name,
                                            DaiValue value) {
    assert(IS_OBJ(receiver));
    DaiObjInstance* instance = AS_INSTANCE(receiver);
    if (!DaiTable_set_if_exist(&instance->fields, name, value)) {
        // todo Exception 将错误也作为一种值返回
        dai_error("PropertyError: '%s' object has no property '%s'", dai_object_ts(receiver),
               name->chars);
        assert(false);
    }
    return NIL_VAL;
...
static bool DaiObjClass_get_method(DaiVM* vm, DaiObjClass *klass, DaiObjString* name, DaiValue *method) {
    while (klass != NULL) {
...
static DaiValue DaiObjClass_get_property(DaiVM* vm, DaiValue receiver, DaiObjString* name) {
    assert(IS_OBJ(receiver));
...
static DaiValue DaiObjClass_set_property(DaiVM* vm, DaiValue receiver, DaiObjString* name,
                                         DaiValue value) {
    assert(IS_OBJ(receiver));
    DaiObjClass* klass = AS_CLASS(receiver);
...
static DaiObjString* allocate_string(DaiVM* vm, char* chars, int length, uint32_t hash) {
...
static void print_function(DaiObjFunction* function) {
...
void dai_print_object(DaiValue value) {
    ...
    case DaiObjType_boundMethod: {
        ...
        dai_log("<bound method %s of <object at %p>>", DaiObjClosure_name(bound_method->method),
               bound_method);
        break;
    }
    ...
    case DaiObjType_string:
        dai_log("%s", AS_CSTRING(value));
        break;
    case DaiObjType_builtinFn:
        dai_log("<builtin-fn %s>", AS_BUILTINFN(value)->name);
        break;
    ...
    default:
        dai_log("Unknown object type %d", OBJ_TYPE(value));
        break;
    }
}
...
// #region 内置函数
static DaiValue builtin_print(__attribute__((unused)) DaiValue receiver, int argc, DaiValue* argv) {
    for (int i = 0; i < argc; i++) {
...
static DaiValue builtin_len(__attribute__((unused)) DaiValue receiver, int argc, DaiValue* argv) {
    if (argc != 1) {
        // todo 将错误也作为一种值
