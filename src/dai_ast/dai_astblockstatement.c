...
static char*
DaiAstBlockStatement_string(DaiAstBase* base, bool recursive) {
...
void
DaiAstBlockStatement_free(DaiAstBase* base, bool recursive) {
    assert(base->type == DaiAstType_BlockStatement);
    DaiAstBlockStatement* block = (DaiAstBlockStatement*)base;
    if (recursive) {
        for (size_t i = 0; i < block->length; i++) {
...
DaiAstBlockStatement_append(DaiAstBlockStatement* block, DaiAstStatement* stmt) {
    if (block->length + 1 > block->size) {
        ...
        block->statements =
            (DaiAstStatement**)dai_realloc(block->statements, sizeof(DaiAstStatement*) * newsize);
        block->size = newsize;
    }
