...
static char *DaiAstVarStatement_string(DaiAstBase *base, bool recursive) {
...
static void DaiAstVarStatement_free(DaiAstBase *base, bool recursive) {
  assert(base->type == DaiAstType_VarStatement);
  DaiAstVarStatement *stmt = (DaiAstVarStatement *)base;
  if (recursive) {
    if (stmt->name != NULL) {
...
DaiAstVarStatement *DaiAstVarStatement_New(DaiAstIdentifier *name,
