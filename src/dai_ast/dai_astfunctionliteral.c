...
static char *DaiAstFunctionLiteral_string(DaiAstBase *base, bool recursive) {
  ...
  DaiStringBuffer_write(sb, KEY_COLOR("type") ": " TYPE_COLOR("DaiAstType_FunctionLiteral") ",\n");
  {
    DaiStringBuffer_write(sb, indent);
...
static void DaiAstFunctionLiteral_free(DaiAstBase *base, bool recursive) {
  assert(base->type == DaiAstType_FunctionLiteral);
  DaiAstFunctionLiteral *expr = (DaiAstFunctionLiteral *)base;
...
static char *DaiAstFunctionLiteral_literal(DaiAstExpression *base) {
  assert(base->type == DaiAstType_FunctionLiteral);
  DaiAstFunctionLiteral *expr = (DaiAstFunctionLiteral *)base;
  DaiStringBuffer *sb = DaiStringBuffer_New();
...
DaiAstFunctionLiteral *DaiAstFunctionLiteral_New(void) {
