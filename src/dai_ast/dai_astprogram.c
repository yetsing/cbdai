...
static char *DaiAstProgram_string(DaiAstBase *base, bool recursive) {
...
static void DaiAstProgram_free(DaiAstBase *base, bool recursive) {
  assert(base->type == DaiAstType_program);
  DaiAstProgram *prog = (DaiAstProgram *)base;
  if (recursive) {
    // 递归释放整个 ast 树
...
void DaiAstProgram_init(DaiAstProgram *program) {
...
void DaiAstProgram_reset(DaiAstProgram *program) {
...
void DaiAstProgram_append(DaiAstProgram *program, DaiAstStatement *stmt) {
  if (program->length + 1 > program->size) {
    size_t newsize = program->size < 8 ? 8 : program->size * 2;
    program->statements =
        dai_realloc(program->statements, sizeof(DaiAstStatement *) * newsize);
