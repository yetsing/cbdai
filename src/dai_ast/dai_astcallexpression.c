...
static char *DaiAstCallExpression_string(DaiAstBase *base, bool recursive) {
  ...
  }
  {
    ...
    if (recursive) {
      for (size_t i = 0; i < call->arguments_count; i++) {
        char *s = call->arguments[i]->string_fn(
            (DaiAstBase *)call->arguments[i], true);
...
static void DaiAstCallExpression_free(DaiAstBase *base, bool recursive) {
  assert(base->type == DaiAstType_CallExpression);
  DaiAstCallExpression *call = (DaiAstCallExpression *)base;
  if (recursive) {
...
static char *DaiAstCallExpression_literal(DaiAstExpression *base) {
  assert(base->type == DaiAstType_CallExpression);
  DaiAstCallExpression *call = (DaiAstCallExpression *)base;
  DaiStringBuffer *sb = DaiStringBuffer_New();
  {
...
DaiAstCallExpression *DaiAstCallExpression_New(void) {
  DaiAstCallExpression *call =
      (DaiAstCallExpression *)dai_malloc(sizeof(DaiAstCallExpression));
