...
void *vm_reallocate(DaiVM *vm, void *pointer, size_t old_size, size_t new_size) {
...
static void vm_free_object(DaiVM *vm, DaiObj *object) {
#ifdef DEBUG_LOG_GC
...
void dai_free_objects(DaiVM *vm) {
...
//#region 垃圾回收
void markObject(DaiVM *vm, DaiObj *object) {
    if (object == NULL) {
        return;
    }
    if (object->is_marked) {
...
void markValue(DaiVM *vm, DaiValue value) {
    if (IS_OBJ(value)) {
        markObject(vm, AS_OBJ(value));
    }
}

static void markArray(DaiVM *vm, DaiValueArray *array) {
    for (int i = 0; i < array->count; i++) {
        markValue(vm, array->values[i]);
    }
}

static void markTable(DaiVM *vm, DaiTable *table) {
...
static void blackenObject(DaiVM *vm, DaiObj *object) {
#ifdef DEBUG_LOG_GC
...
static void markRoots(DaiVM *vm) {
    // 标记栈
...
static void traceReferences(DaiVM *vm) {
