...
static void Parser_FreeCallArguments(DaiAstExpression **args,
                                     size_t arg_count) {
...
// 返回 NULL 表示解析失败
static DaiAstExpression **Parser_parseCallArguments(Parser *p,
                                                    size_t *arg_count) {
  size_t arg_size = 4;
  DaiAstExpression **args =
      (DaiAstExpression **)dai_malloc(sizeof(DaiAstExpression *) * arg_size);
  *arg_count = 0;
  ...
  while (Parser_peekTokenIs(p, DaiTokenType_comma)) {
    ...
    }
    {
      if (*arg_count == arg_size) {
        arg_size *= 2;
        args = (DaiAstExpression **)dai_realloc(
            args, sizeof(DaiAstExpression *) * arg_size);
      }
    }
...
static DaiAstExpression *Parser_parseCallExpression(Parser *p,
