...
void DaiCompiler_addName(DaiCompiler* compiler, const char* name, int back) {
...
static int DaiCompiler_addConstant(DaiCompiler* compiler, DaiValue value);
static int DaiCompiler_emit(DaiCompiler* compiler, DaiOpCode op, int line);
static int DaiCompiler_emit1(DaiCompiler* compiler, DaiOpCode op, uint8_t operand, int line);
static int DaiCompiler_emit2(DaiCompiler* compiler, DaiOpCode op, uint16_t operand, int line);
static int DaiCompiler_emit3(DaiCompiler* compiler, DaiOpCode op, uint16_t operand1,
                             uint8_t operand2, int line);
static void DaiCompiler_patchJump(DaiCompiler* compiler, int offset);
static void DaiCompiler_reset(DaiCompiler* compiler);
static DaiCompileError* DaiCompiler_compile(DaiCompiler* compiler, DaiAstBase* node);

static DaiCompileError* DaiCompiler_loadSymbol(DaiCompiler* compiler, DaiSymbol* symbol,
                                               int start_line) {
    switch (symbol->type) {
    ...
    case DaiSymbolType_self: {
        if (compiler->type != FunctionType_method && compiler->type != FunctionType_classMethod) {
            return DaiCompileError_New("Cannot use 'self' outside of a method", compiler->filename,
                                       start_line, 0);
        }
        // self 固定在第 0 个
...
static void DaiCompiler_init(DaiCompiler* compiler, DaiObjFunction* function,
...
static DaiCompileError* DaiCompiler_compileFunction(DaiCompiler* compiler, DaiAstBase* node) {
    // 函数名字
    const char* name;
    ...
    DaiCompiler_emit3(compiler, DaiOpClosure, DaiCompiler_addConstant(compiler, OBJ_VAL(function)),
                      num_free, start_line);
...
// 更新符号表
static DaiCompileError* DaiCompiler_extractSymbol(DaiCompiler* compiler, DaiAstBase* node) {
    char* name = NULL;
    int line = 0;
    int column = 0;
    switch (node->type) {
...
static DaiCompileError* DaiCompiler_compile(DaiCompiler* compiler, DaiAstBase* node) {
    ...
    case DaiAstType_VarStatement: {
        ...
        if (DaiSymbolTable_isDefined(compiler->symbolTable, stmt->name->value)) {
            return DaiCompileError_Newf(compiler->filename, stmt->name->start_line,
                                        stmt->name->start_column, "symbol '%s' already defined",
                                        stmt->name->value);
        }
        ...
        case DaiSymbolType_local: {
            if (symbol.index >= UINT8_MAX) {
                return DaiCompileError_Newf(compiler->filename, stmt->name->start_line,
                                            ...
                                            "too many local symbols(MAX=%d)", UINT8_MAX);
            }
    ...
    case DaiAstType_AssignStatement: {
        ...
        case DaiAstType_Identifier: {
            ...
            if (!found || !symbol.defined) {
                return DaiCompileError_Newf(compiler->filename, ident->start_line,
                                            ident->start_column, "undefined variable: '%s'",
                                            ident->value);
            }
            switch (symbol.type) {
            case DaiSymbolType_global: {
        ...
        case DaiAstType_DotExpression: {
            ...
            DaiObjString *name = dai_copy_string(compiler->vm, expr->name->value, strlen(expr->name->value));
    ...
    case DaiAstType_ReturnStatement: {
        ...
        if (compiler->type == FunctionType_script) {
            return DaiCompileError_New("Can't return from top-level code", compiler->filename,
    ...
    case DaiAstType_FunctionStatement: {
        ...
        if (DaiSymbolTable_isDefined(compiler->symbolTable, stmt->name)) {
            return DaiCompileError_Newf(compiler->filename, stmt->start_line, stmt->start_column,
                                        "symbol '%s' already defined", stmt->name);
        }
        ...
        case DaiSymbolType_local: {
            if (symbol.index >= UINT8_MAX) {
                return DaiCompileError_Newf(compiler->filename, stmt->start_line,
                                            stmt->start_column, "too many local symbols(MAX=%d)",
                                            UINT8_MAX);
            }
            DaiCompiler_emit1(compiler, DaiOpSetLocal, symbol.index, stmt->start_line);
            ADD_LOCAL_NAME(compiler, symbol.name);
    ...
    case DaiAstType_InsVarStatement: {
        ...
        if (DaiTable_has(&compiler->propertys, name)) {
            return DaiCompileError_Newf(compiler->filename, stmt->name->start_line,
                                        stmt->name->start_column, "property '%s' already defined",
                                        stmt->name->value);
        }
    ...
    case DaiAstType_MethodStatement: {
        ...
        if (DaiTable_has(&compiler->propertys, name)) {
            return DaiCompileError_Newf(compiler->filename, stmt->start_line, stmt->start_column,
                                        "property '%s' already defined", stmt->name);
        }
        DaiCompileError* err = DaiCompiler_compileFunction(compiler, node);
        if (err != NULL) {
            return err;
    ...
    case DaiAstType_ClassVarStatement: {
        ...
        DaiObjString* name = dai_copy_string(compiler->vm, stmt->name->value, strlen(stmt->name->value));
        if (DaiTable_has(&compiler->propertys, name)) {
            return DaiCompileError_Newf(compiler->filename, stmt->start_line, stmt->start_column,
                                        "property '%s' already defined", stmt->name->value);
        }
        DaiTable_set(&compiler->propertys, name, INTEGER_VAL(0));
        int index = DaiChunk_addConstant(&compiler->function->chunk, OBJ_VAL(name));
    ...
    case DaiAstType_SelfExpression: {
        ...
        if (compiler->type != FunctionType_method && compiler->type != FunctionType_classMethod) {
            return DaiCompileError_Newf(compiler->filename, expr->start_line, expr->start_column,
    ...
    case DaiAstType_InfixExpression: {
        ...
        if (strcmp(expr->operator, "+") == 0) {
            ...
            return DaiCompileError_Newf(compiler->filename, expr->start_line, expr->start_column,
                                        "unknown operator: '%s'", expr->operator);
        }
        break;
    }
    ...
    case DaiAstType_Identifier: {
        ...
        if (!found || (compiler->type == FunctionType_script && !symbol.defined)) {
            return DaiCompileError_Newf(compiler->filename, id->start_line, id->start_column,
                                        "undefined variable: '%s'", id->value);
        }
    ...
    case DaiAstType_IntegerLiteral: {
        ...
        DaiCompiler_emit2(compiler, DaiOpConstant, DaiCompiler_addConstant(compiler, integer),
                          lit->start_line);
        break;
    }
    case DaiAstType_StringLiteral: {
        ...
        DaiCompiler_emit2(compiler, DaiOpConstant,
                          DaiCompiler_addConstant(compiler, OBJ_VAL(string)), lit->start_line);
        break;
    }
...
static void DaiCompiler_reset(DaiCompiler* compiler) { DaiTable_reset(&compiler->propertys); }
...
DaiCompileError* dai_compile(DaiAstProgram* program, DaiObjFunction* function, DaiVM* vm) {
