...
void SymbolMap_init(SymbolMap *table) {
  table->count = 0;
  table->capacity = 0;
  table->entries = NULL;
}

void SymbolMap_reset(SymbolMap *table) {
...
static SymbolEntry *find_entry(SymbolEntry *entries, int capacity, const char *key, int length) {
...
bool SymbolMap_get(SymbolMap *table,const char *key, DaiSymbol *value) {
  if (table->count == 0)
    return false;
...
static void adjust_capacity(SymbolMap *table, int capacity) {
...
bool SymbolMap_set(SymbolMap *table, char *key, DaiSymbol value) {
...
static DaiSymbol SymbolMapIterator_next(SymbolMapIterator *iterator) {
...
__attribute__ ((unused))
static SymbolMapIterator SymbolMap_newIterator(SymbolMap *table)  {
...
DaiSymbolTable *DaiSymbolTable_New() {
...
DaiSymbolTable *DaiSymbolTable_NewEnclosed(DaiSymbolTable *outer) {
...
DaiSymbolTable *DaiSymbolTable_NewFunction(DaiSymbolTable *outer) {
  DaiSymbolTable *table = DaiSymbolTable_New();
  table->depth = outer->depth + 1;
  table->outer = outer;
  table->num_symbols_of_outer = 0;
...
void DaiSymbolTable_free(DaiSymbolTable *table) {
  SymbolMap_reset(&table->store);
  FREE(DaiSymbolTable, table);
}

DaiSymbol DaiSymbolTable_predefine(DaiSymbolTable *table, const char *name) {
  char *cname = strdup(name);
  DaiSymbol symbol = {cname, table->depth, table->num_symbols + table->num_symbols_of_outer, false};
  if (table->depth == 0) {
...
DaiSymbol DaiSymbolTable_define(DaiSymbolTable *table, const char *name) {
  DaiSymbol symbol;
...
DaiSymbol DaiSymbolTable_defineBuiltin(DaiSymbolTable *table, int index, const char *name) {
  char *cname = strdup(name);
...
DaiSymbol DaiSymbolTable_defineSelf(DaiSymbolTable *table) {
...
DaiSymbol DaiSymbolTable_defineFree(DaiSymbolTable *table, DaiSymbol original) {
  table->free_symbols[table->num_free] = original;
  char *cname = strdup(original.name);
  DaiSymbol symbol = (DaiSymbol) {cname, original.depth, table->num_free, true, DaiSymbolType_free};
  bool isNewKey = SymbolMap_set(&table->store, symbol.name, symbol);
  table->num_free++;
  assert(isNewKey);
  return symbol;
...
bool DaiSymbolTable_resolve(DaiSymbolTable *table, const char *name, DaiSymbol *symbol) {
  bool found = SymbolMap_get(&table->store, name, symbol);
  if (!found && table->outer != NULL) {
    found = DaiSymbolTable_resolve(table->outer, name, symbol);
    if (found && (symbol->type != DaiSymbolType_builtin && symbol->type != DaiSymbolType_global) && table->function_depth > table->outer->function_depth) {
...
bool DaiSymbolTable_isDefined(DaiSymbolTable *table, const char *name) {
  DaiSymbol symbol;
  return SymbolMap_get(&table->store, name, &symbol) && symbol.defined;
}

int DaiSymbolTable_count(DaiSymbolTable *table) {
  return table->num_symbols;
}

DaiSymbol * DaiSymbolTable_getFreeSymbols(DaiSymbolTable *table, int *free_symbol_count) {
