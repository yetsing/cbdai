...
bool dai_value_is_truthy(DaiValue value) {
    switch (value.type) {
...
void DaiVM_resetStack(DaiVM* vm) {
    vm->stack_top = vm->stack;
    vm->frameCount = 0;
}

void DaiVM_init(DaiVM* vm) {
    DaiVM_resetStack(vm);
    {
        // 初始化栈
...
static DaiValue DaiVM_peek(DaiVM* vm, int distance) { return vm->stack_top[-1 - distance]; }

static void concatenate_string(DaiVM* vm, DaiValue v1, DaiValue v2) {
...
static DaiRuntimeError* DaiVM_call(DaiVM* vm, DaiObjFunction* function, int argCount) {
    if (argCount != function->arity) {
        CallFrame* frame = &vm->frames[vm->frameCount - 1];
        DaiChunk* chunk = &frame->function->chunk;
        return DaiRuntimeError_Newf(chunk->filename,
                                    DaiChunk_getLine(chunk, (int)(frame->ip - chunk->code)), 0,
                                    "Expected %d arguments but got %d", function->arity, argCount);
    }
    if (vm->frameCount == FRAMES_MAX) {
        CallFrame* frame = &vm->frames[vm->frameCount - 1];
        DaiChunk* chunk = &frame->function->chunk;
        return DaiRuntimeError_New("Stack overflow", chunk->filename,
                                   DaiChunk_getLine(chunk, (int)(frame->ip - chunk->code)), 0);
    }
    // new frame
...
static DaiRuntimeError* DaiVM_callValue(DaiVM* vm, DaiValue callee, int argCount);
// 在实例的 init 方法后调用
static DaiValue DaiVM_post_init(DaiVM* vm) {
...
static DaiRuntimeError* DaiVM_callClass(DaiVM* vm, DaiObjClass* klass, int argCount) {
...
static DaiRuntimeError* DaiVM_callValue(DaiVM* vm, DaiValue callee, int argCount) {
    if (IS_OBJ(callee)) {
        ...
        case DaiObjType_builtinFn: {
            ...
            DaiValue result = func(DaiVM_peek(vm, argCount), argCount, vm->stack_top - argCount);
    ...
    return DaiRuntimeError_Newf(chunk->filename,
                                DaiChunk_getLine(chunk, (int)(frame->ip - chunk->code)), 0,
                                "'%s' object is not callable", dai_value_ts(callee));
}

static DaiRuntimeError* DaiVM_dorun(DaiVM* vm) {
    CallFrame* frame = &vm->frames[vm->frameCount - 1];
    DaiChunk* chunk = &frame->function->chunk;
    // 先取值，再自增
    ...
    for (; frame->ip < chunk->code + chunk->count;) {
        ...
        case DaiOpAdd: {
            ...
            if (!IS_INTEGER(a) || !IS_INTEGER(b)) {
                return DaiRuntimeError_Newf(
                    chunk->filename, DaiChunk_getLine(chunk, (int)(frame->ip - chunk->code)), 0,
                    "TypeError: unsupported operand type(s) for +: '%s' and '%s'", dai_value_ts(a),
                    dai_value_ts(b));
            }
            DaiVM_pop(vm);
            DaiVM_pop(vm);
        ...
        case DaiOpMinus: {
            ...
            if (!IS_INTEGER(a)) {
                return DaiRuntimeError_Newf(
                    chunk->filename, DaiChunk_getLine(chunk, (int)(frame->ip - chunk->code)), 0,
                    "TypeError: unsupported operand type(s) for -: '%s'", dai_value_ts(a));
            }
        ...
        case DaiOpClosure: {
            ...
            for (int i = 0; i < free_var_count; i++) {
                frees[i] = DaiVM_peek(vm, free_var_count - i - 1); // peek 里面 -1 ，所以这里要 -1
            }
        ...
        case DaiOpGetProperty: {
            ...
            if (IS_OBJ(receiver)) {
                ...
                return DaiRuntimeError_Newf(
                    chunk->filename, DaiChunk_getLine(chunk, (int)(frame->ip - chunk->code)), 0,
                    "'%s' object has no property1 '%s'", dai_value_ts(receiver), name->chars);
            }
            break;
        }
        case DaiOpSetProperty: {
            ...
            if (IS_OBJ(receiver)) {
                ...
                return DaiRuntimeError_Newf(
                    chunk->filename, DaiChunk_getLine(chunk, (int)(frame->ip - chunk->code)), 0,
                    "'%s' object has no property2 '%s'", dai_value_ts(receiver), name->chars);
            }
            break;
        }
        ...
        case DaiOpGetSuperProperty: {
            ...
            uint16_t name_index = READ_UINT16();
            DaiObjString* name = AS_STRING(chunk->constants.values[name_index]);
            DaiObjBoundMethod *bound_method = DaiObjClass_get_bound_method(vm, frame->function->superclass, name, frame->slots[0]);
        ...
        default: {
            return DaiRuntimeError_Newf(chunk->filename,
                                        DaiChunk_getLine(chunk, (int)(frame->ip - chunk->code)), 0,
                                        "Unknown opcode: %s(%d)", dai_opcode_name(op), op);
        }
        }
    }
    return NULL;
...
DaiRuntimeError* DaiVM_run(DaiVM* vm, DaiObjFunction* function) {
...
void DaiVM_push(DaiVM* vm, DaiValue value) {
    *vm->stack_top = value;
    vm->stack_top++;
    assert(vm->stack_top <= vm->stack + STACK_MAX);
}
DaiValue DaiVM_pop(DaiVM* vm) {
    vm->stack_top--;
    return *vm->stack_top;
}
DaiValue DaiVM_stackTop(DaiVM* vm) { return vm->stack_top[-1]; }

// #region 用于测试的函数
DaiValue DaiVM_lastPopedStackElem(DaiVM* vm) { return *vm->stack_top; }
bool DaiVM_isEmptyStack(DaiVM* vm) {
    // 栈里面会留下一个脚本对应的函数
