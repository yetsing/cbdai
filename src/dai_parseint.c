...
static bool dai_underscore_ok(const char *str, size_t len) {
  // saw 追踪我们看到的最后一个字符
  // ^ 表示开始
  // 0 表示数字或者进制前缀
  // _ 表示下划线
  ...
  // 去掉前面的进制前缀
  if (len >= 2 &&
      (str[0] == '0' && (str[1] == 'b' || str[1] == 'B' || str[1] == 'o' ||
...
static uint64_t dai_parseuint(const char *str, int base, char **error) {
  ...
  *error = NULL;
  ...
  switch (base) {
  case 10:
    cutoff = UINT64_MAX / 10 + 1;
    break;
  case 16:
    cutoff = UINT64_MAX / 16 + 1;
    break;
  default:
    cutoff = UINT64_MAX / base + 1;
    break;
  }
...
// 代码思路参照
// https://cs.opensource.google/go/go/+/refs/tags/go1.22.3:src/strconv/atoi.go
int64_t dai_parseint(const char *str, int base, char **error) {
  daiassert(str != NULL, "str is NULL");
  *error = NULL;
