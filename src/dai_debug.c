...
void DaiChunk_disassemble(DaiChunk* chunk, const char* name) {
    printf("== %s ==\n", name);
...
static int constant_instruction(const char* name, DaiChunk* chunk, int offset) {
...
static int jump_instruction(const char* name, DaiChunk* chunk, int offset) {
...
static int global_instruction(const char* name, DaiChunk* chunk, int offset) {
...
static int call_instruction(const char* name, DaiChunk* chunk, int offset) {
    int argCount = DaiChunk_read(chunk, offset + 1);
    printf("%s %d\n", name, argCount);
    return offset + 2;
}

static int local_instruction(const char* name, DaiChunk* chunk, int offset) {
...
static int builtin_instruction(const char* name, DaiChunk* chunk, int offset) {
    uint8_t index = DaiChunk_read(chunk, offset + 1);
    printf("%-16s %4d %s\n", name, index, builtin_funcs[index].name);
    return offset + 2;
}

static int closure_instruction(const char* name, DaiChunk* chunk, int offset) {
...
static int free_instruction(const char* name, DaiChunk* chunk, int offset) {
    int index = DaiChunk_read(chunk, offset + 1);
    printf("%-16s %4d", name, index);
    fflush(stdout);
#ifdef DISASSEMBLE_VARIABLE_NAME
...
static int property_instruction(const char* name, DaiChunk* chunk, int offset) {
    uint16_t constant = DaiChunk_readu16(chunk, offset + 1);
    printf("%-16s %4d '", name, constant);
    dai_print_value(chunk->constants.values[constant]);
    printf("'\n");
...
int DaiChunk_disassembleInstruction(DaiChunk* chunk, int offset) {
    printf("%04d ", offset);
    ...
    case DaiOpGreaterThan:
        return simple_instruction("OP_GREATER_THAN", offset);
    ...
    case DaiOpReturnValue:
        return simple_instruction("OP_RETURN_VALUE", offset);
    case DaiOpReturn:
    ...
    case DaiOpGetBuiltin:
        return builtin_instruction("OP_GET_BUILTIN", chunk, offset);

    case DaiOpClosure:
        return closure_instruction("OP_CLOSURE", chunk, offset);

    case DaiOpGetFree:
        return free_instruction("OP_GET_FREE", chunk, offset);

    case DaiOpClass:
        return constant_instruction("OP_CLASS", chunk, offset);
    case DaiOpDefineField:
        return property_instruction("OP_DEFINE_FIELD", chunk, offset);
    case DaiOpDefineMethod:
        return property_instruction("OP_DEFINE_METHOD", chunk, offset);
    ...
    case DaiOpGetProperty:
        return property_instruction("OP_GET_PROPERTY", chunk, offset);
    case DaiOpSetProperty:
        return property_instruction("OP_SET_PROPERTY", chunk, offset);
