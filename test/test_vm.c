...
test_builtin_functions(__attribute__((unused)) const MunitParameter params[],
    ...
    DaiVMTestCase tests[] = {
        ...
        // time
        {"var t = time.time(); t > 1737965587;", dai_true},
        {"var t = time.timef(); t > 1737965587;", dai_true},
        {"var t1 = time.time(); time.sleep(1); var t2 = time.time(); t2 > t1;", dai_true},
        {"var t1 = time.time(); time.sleep(1.1); var t2 = time.time(); t2 > t1;", dai_true},
        ...
        {"var m = range(9, -1, -1); var sum = 0; for(var i, e in m) { sum += e; }; sum;",
         INTEGER_VAL(45)},
        {"math.sqrt(4);", FLOAT_VAL(2.0)},
        {"math.sin(0);", FLOAT_VAL(0)},
        {"math.cos(0);", FLOAT_VAL(1)},
        {"abs(0);", INTEGER_VAL(0)},
        {"abs(1);", INTEGER_VAL(1)},
        {"abs(-1);", INTEGER_VAL(1)},
...
test_error(__attribute__((unused)) const MunitParameter params[],
    ...
    const DaiVMTestCase tests[] = {
        ...
        {
            "time.time(1);",
            OBJ_VAL(DaiObjError_Newf(&vm, "time.time() expected no arguments, but got 1")),
        },
        {
            "time.timef(1);",
            OBJ_VAL(DaiObjError_Newf(&vm, "time.timef() expected no arguments, but got 1")),
        },
        {
            "time.sleep(1, 1);",
            OBJ_VAL(DaiObjError_Newf(&vm, "time.sleep() expected 1 argument, but got 2")),
        },
        {
            "time.sleep('1');",
            OBJ_VAL(
                DaiObjError_Newf(&vm, "time.sleep() expected number arguments, but got string")),
        },
        {
            "range(1, 2, 3, 4);",
        ...
        {
            "math.sqrt(1, 1);",
            OBJ_VAL(DaiObjError_Newf(&vm, "math.sqrt() expected 1 argument, but got 2")),
        },
        {
            "math.sqrt('1');",
            OBJ_VAL(DaiObjError_Newf(&vm, "math.sqrt() expected number arguments, but got string")),
        },
        {
            "math.sin(1, 1);",
            OBJ_VAL(DaiObjError_Newf(&vm, "math.sin() expected 1 argument, but got 2")),
        },
        {
            "math.sin('1');",
            OBJ_VAL(DaiObjError_Newf(&vm, "math.sin() expected number arguments, but got string")),
        },
        {
            "math.cos(1, 1);",
            OBJ_VAL(DaiObjError_Newf(&vm, "math.cos() expected 1 argument, but got 2")),
        },
        {
            "math.cos('1');",
            OBJ_VAL(DaiObjError_Newf(&vm, "math.cos() expected number arguments, but got string")),
        },
        {
            "abs(1, 1);",
